...
bool SearchForCopy(const char *strPathFrom, const char *strPathTo, const int &index);	// search for files which doesn't exist in a destination folder
bool DeletingFolder(const char *strFolder);												// remove folder
bool SearchForDeleting(char *strPathFrom, char *strPathTo, const int &index);			// search for "extra" files in a destination
